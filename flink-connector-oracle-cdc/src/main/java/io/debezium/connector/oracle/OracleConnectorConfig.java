...
public class OracleConnectorConfig extends HistorizedRelationalDatabaseConnectorConfig {
    ...
    public static final Field XSTREAM_SERVER_NAME =
            Field.create(DATABASE_CONFIG_PREFIX + "out.server.name")
    ...
    public static final Field TABLENAME_CASE_INSENSITIVE =
            Field.create("database.tablename.case.insensitive")
    ...
    public static final Field ORACLE_VERSION =
            Field.createInternal("database.oracle.version")
                    .withDisplayName("Oracle version, 11 or 12+")
                    .withType(Type.STRING)
                    .withImportance(Importance.LOW)
                    .withDescription(
                            "Deprecated: For default Oracle 12+, use default pos_version value v2, for Oracle 11, use pos_version value v1.");
    ...
    public static final Field CONNECTOR_ADAPTER =
            Field.create(DATABASE_CONFIG_PREFIX + "connection.adapter")
    ...
    public static final Field LOG_MINING_HISTORY_RECORDER_CLASS =
            Field.create("log.mining.history.recorder.class")
    ...
    public static final Field LOG_MINING_HISTORY_RETENTION =
            Field.create("database.history.retention.hours")
                    ...
                    .withDefault(0)
                    .withDescription(
                            "Hours to keep Log Mining history.  By default, no history is retained.");
     ...
     * which indicates the connector configuration does not specify a value and should therefore be
     * resolved by the {@link OracleConnection}.
     *
        ...
        return new HistoryRecordComparator() {
            ...
            protected boolean isPositionAtOrBefore(Document recorded, Document desired) {
                ...
                if (getAdapter() == OracleConnectorConfig.ConnectorAdapter.XSTREAM) {
                    final LcrPosition recordedPosition =
                            LcrPosition.valueOf(recorded.getString(SourceInfo.LCR_POSITION_KEY));
                    final LcrPosition desiredPosition =
                            LcrPosition.valueOf(desired.getString(SourceInfo.LCR_POSITION_KEY));
                    recordedScn =
                            recordedPosition != null
                                    ? recordedPosition.getScn()
                                    : resolveScn(recorded);
                    desiredScn =
                            desiredPosition != null
                                    ? desiredPosition.getScn()
                                    : resolveScn(desired);
    ...
    public enum SnapshotMode implements EnumeratedValue {
        ...
        /**
         * Perform a snapshot of only the database schemas (without data) and then begin reading the
         * redo log at the current redo log position. This can be used for recovery only if the
         * connector has existing offsets and the database.history.kafka.topic does not exist
         * (deleted). This recovery option should be used with care as it assumes there have been no
         * schema changes since the connector last stopped, otherwise some events during the gap may
         * be processed with an incorrect schema and corrupted.
         */
        ...
        private SnapshotMode(
                String value, boolean includeData, boolean shouldSnapshotOnSchemaError) {
            this.value = value;
    ...
    /** Data capture strategy for the log-Miner connector. */
    ...
    /** DML parser for the log-miner connector. */
    ...
    private static class SystemTablesPredicate implements TableFilter {
        ...
        private boolean isFlushTable(TableId id) {
            ...
            return id.table().equalsIgnoreCase(SqlUtils.LOGMNR_FLUSH_TABLE)
                    && id.schema().equalsIgnoreCase(schema);
        }
    }
    ...
    /**
     * @return the duration for which long running transactions are permitted in the transaction
     *     buffer between log switches
     */
    ...
    public static int validateOutServerName(
            Configuration config, Field field, ValidationOutput problems) {
        if (ConnectorAdapter.XSTREAM.equals(
                ConnectorAdapter.parse(config.getString(CONNECTOR_ADAPTER)))) {
    ...
    private static Boolean resolveTableNameCaseInsensitivity(Configuration config) {
        if (config.hasKey(TABLENAME_CASE_INSENSITIVE.name())) {
            LOGGER.warn(
                    ...
                    TABLENAME_CASE_INSENSITIVE.name());
